...
echo -e "${GREEN}Install additional package"
apt install curl wget bash sudo keepalived libipset*

echo -e "${GREEN}remove existing cloudflared"
...
echo -e "${GREEN}install cloudflared"
wget https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-amd64.deb
sudo apt-get install ./cloudflared-linux-amd64.deb
cloudflared -v

echo -e "${GREEN}adduser cloudflared"
sudo useradd -s /usr/sbin/nologin -r -M cloudflared

echo -e "${GREEN}created /etc/default/cloudflared"
...
echo -e "${GREEN}Make chown /etc/default/cloudflared and /usr/local/bin/cloudflared"
sudo chown cloudflared:cloudflared /etc/default/cloudflared
sudo chown cloudflared:cloudflared /usr/local/bin/cloudflared

echo -e "${GREEN}created /etc/systemd/system/cloudflared.service"
printf "[Unit] \nDescription=cloudflared DNS over HTTPS proxy \nAfter=syslog.target network-online.target \n \n[Service] \nType=simple \nUser=cloudflared \nEnvironmentFile=/etc/default/cloudflared \nExecStart=/usr/local/bin/cloudflared proxy-dns $CLOUDFLARED_OPTS \nRestart=on-failure \nRestartSec=10 \nKillMode=process \n \n[Install] \nWantedBy=multi-user.target" >> /etc/systemd/system/cloudflared.service

echo -e "${GREEN}Running Service Cloudflare"
...
echo -e "${GREEN}Uninstall Pi-Hole"
...
echo -e "${GREEN}Install Pi-Hole"
wget -O basic-install.sh https://install.pi-hole.net
sudo bash basic-install.sh

echo -e "${GREEN}Pi-hole 1"
echo -e "${GREEN}Role: Master or Active server"
echo -e "${GREEN}Hostname: pihole-dns-01"
echo -e "${GREEN}IP: 192.168.1.253"

echo -e "${GREEN}Pi-hole 2 (THIS)"
echo -e "${GREEN}Role: Backup or Standby server"
echo -e "${GREEN}Hostname: pihole-dns-02"
echo -e "${GREEN}IP: 192.168.1.252"

echo -e "${GREEN}High Availability:"
echo -e "${GREEN}IP: 192.168.1.254"

echo -e "${GREEN}Setting pi-hole gemini"

echo -e "${GREEN}Setting gemini file"
#cd /usr/local/bin
...
echo -e "${GREEN}Create an SSH key"
ssh-keygen -t rsa

echo -e "${GREEN}Send the SSH key to your ‘other’ Pi-hole machine."
ssh-copy-id root@192.168.1.253

echo -e "${GREEN}integrate the script into Pi-hole"
...
echo -e "${GREEN}enable keepalived"
sudo systemctl enable keepalived.service

echo -e "${GREEN}create the pihole-FTL service check script"
